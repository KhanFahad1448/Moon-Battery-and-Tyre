...
const textY = useTransform(scrollYProgress, [0, 1], ["0%", "10%"]);
  ...
  return (
    ...
    {/* Categories */}
    <section className="container mx-auto grid gap-4 px-6 py-12 sm:gap-6 sm:py-24 lg:grid-cols-2">
      {[
      {
        to: "/tyres",
        img: images.heroTyre,
      ...
      <Reveal key={cat.to} delay={i * 0.12}>
         <Link
           to={cat.to}
          className="group relative block h-52 overflow-hidden rounded-lg border border-border sm:h-80 lg:h-[420px]"
            >
           <img
            src={cat.img}
            alt={cat.title}
         ...
         <div className="absolute inset-0 bg-gradient-to-t from-background via-background/40 to-transparent" />
           <div className="absolute inset-x-0 bottom-0 p-4 sm:p-8">
             <p className="text-[9px] uppercase tracking-[0.25em] text-ember sm:text-[10px] sm:tracking-[0.35em]">{cat.eyebrow}</p>
            <h2 className="mt-1 text-3xl leading-none sm:mt-2 sm:text-6xl">{cat.title}</h2>
               <p className="mt-1.5 line-clamp-1 max-w-sm text-xs text-muted-foreground sm:mt-3 sm:line-clamp-none sm:text-sm">{cat.text}</p>
            <span className="mt-2 inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-ember sm:mt-5 sm:gap-2 sm:text-xs sm:tracking-[0.2em]">
              Explore
              <ArrowRight size={12} className="transition-transform duration-300 group-hover:translate-x-2 sm:size-[14px]" />
             </span>
             </div>
            </Link>
          </Reveal>
...
{/* Promises */}
    <section className="container mx-auto px-6 py-12 sm:py-24">
        <Reveal className="mb-6 max-w-2xl sm:mb-12">
         <p className="text-[9px] uppercase tracking-[0.25em] text-ember sm:text-[10px] sm:tracking-[0.35em]">Why Moon</p>
         <h2 className="mt-2 text-3xl sm:text-5xl md:text-6xl">NO SHORTCUTS, EVER</h2>
       </Reveal>
       <Stagger className="grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-border bg-border lg:grid-cols-4">
          {promises.map((p) => (
        <StaggerItem key={p.title}>
        <div className="group h-full bg-surface p-3 transition-colors duration-500 hover:bg-surface-2 sm:p-8">
          <p.icon size={18} className="text-ember transition-transform duration-500 group-hover:scale-110 sm:size-[26px]" />
          <h3 className="mt-2 text-sm leading-tight sm:mt-5 sm:text-2xl sm:leading-none">{p.title}</h3>
          <p className="mt-1.5 line-clamp-2 text-xs leading-relaxed text-muted-foreground sm:mt-3 sm:line-clamp-none sm:text-sm">{p.text}</p>
        </div>
        </StaggerItem>
         ))}
       </Stagger>
      ...
      {/* Services strip */}
<section className="container mx-auto px-6 py-12 sm:py-24">
  <Reveal className="mb-6 flex flex-wrap items-end justify-between gap-3 sm:mb-12 sm:gap-6">
    <div>
      <p className="text-[9px] uppercase tracking-[0.25em] text-ember sm:text-[10px] sm:tracking-[0.35em]">Workshop</p>
      <h2 className="mt-2 text-3xl sm:text-5xl md:text-6xl">MORE THAN A SHOP</h2>
    </div>
    <Link to="/services" className="text-[10px] font-bold uppercase tracking-[0.15em] text-ember ember-underline sm:text-xs sm:tracking-[0.2em]">
      All services
    </Link>
  </Reveal>
        ...
        {services.slice(0, 4).map((s) => (
          <StaggerItem key={s.slug}>
           <Link
            to={`/services/${s.slug}`}
            className="hover-lift block h-full rounded-lg border border-border bg-surface p-3 sm:p-7"
        >
          <p className="text-[9px] uppercase tracking-[0.2em] text-ember sm:text-[10px] sm:tracking-[0.28em]">{s.duration}</p>
          <h3 className="mt-1.5 text-sm leading-tight sm:mt-3 sm:text-2xl sm:leading-none">{s.title}</h3>
          <p className="mt-1.5 line-clamp-2 text-xs text-muted-foreground sm:mt-3 sm:line-clamp-none sm:text-sm">{s.summary}</p>
          <p className="mt-2 text-xs font-semibold text-foreground sm:mt-5 sm:text-sm">{s.price}</p>
        </Link>
      </StaggerItem>
        ))}
       </Stagger>
      ...
      {/* Testimonials */}
     <section className="border-y border-border bg-surface/20 py-12 sm:py-24">
      <div className="container mx-auto px-6">
       <Reveal className="mb-6 sm:mb-12">
         <p className="text-[9px] uppercase tracking-[0.25em] text-ember sm:text-[10px] sm:tracking-[0.35em]">4.9 average · 3,400 reviews</p>
      <  h2 className="mt-2 text-3xl sm:text-5xl md:text-6xl">DRIVERS TALK</h2>
       </Reveal>
    <   Stagger className="grid gap-3 sm:gap-6 md:grid-cols-3">
         {testimonials?.slice(0, 3).map((t) => (
        <StaggerItem key={t.name}>
          <figure className="hover-lift flex h-full flex-col rounded-lg border border-border bg-surface p-4 sm:p-8">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-ember text-xs font-bold text-primary-foreground sm:h-10 sm:w-10 sm:text-sm">
                  {t.name?.[0]}
                </span>
                <figcaption className="text-xs text-muted-foreground sm:text-sm">
                  ...
                  <span>{t.car} · {t.city}</span>
                </figcaption>
              </div>
              <div className="flex shrink-0 gap-0.5">
                {Array.from({ length: t.rating }).map((_, i) => (
                  <Star key={i} size={11} className="fill-ember text-ember sm:size-[13px]" />
                ))}
              </div>
            </div>
            <blockquote className="mt-3 line-clamp-3 text-sm leading-relaxed text-muted-foreground sm:mt-5 sm:line-clamp-none sm:text-lg sm:text-foreground">
              "{t.quote}"
            </blockquote>
          </figure>
        </StaggerItem>
      ...
      <section className="container mx-auto px-6 py-12 sm:py-24">
         <Reveal className="mb-6 flex flex-wrap items-end justify-between gap-3 sm:mb-12 sm:gap-6">
         <div>
           <p className="text-[9px] uppercase tracking-[0.25em] text-ember sm:text-[10px] sm:tracking-[0.35em]">Journal</p>
           <h2 className="mt-2 text-3xl sm:text-5xl md:text-6xl">KNOW YOUR RUBBER</h2>
         </div>
          <Link to="/blog" className="text-[10px] font-bold uppercase tracking-[0.15em] text-ember ember-underline sm:text-xs sm:tracking-[0.2em]">
             Read all
           </Link>
         </Reveal>
          <Stagger className="grid grid-cols-2 gap-3 sm:gap-6 md:grid-cols-3">
            {posts.slice(0, 3).map((p) => (
          <StaggerItem key={p.slug}>
            <Link
             to={`/blog/${p.slug}`}
             className="hover-lift group block h-full rounded-lg border border-border bg-surface p-3 sm:p-8"
          >
             <p className="text-[9px] uppercase tracking-[0.2em] text-ember sm:text-[10px] sm:tracking-[0.28em]">
               {p.category} · {p.readTime}
             </p>
             <h3 className="mt-1.5 text-sm leading-tight sm:mt-3 sm:text-3xl sm:leading-none">{p.title}</h3>
             <p className="mt-1.5 line-clamp-2 text-xs leading-relaxed text-muted-foreground sm:mt-4 sm:line-clamp-none sm:text-sm">{p.excerpt}</p>
          </Link>
          </StaggerItem>
          ))}
         </Stagger>
